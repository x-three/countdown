jQuery(function($) {
    ...
    var oParams = new window.webapp.Params(),
        ny = new Date((new Date()).getFullYear() + 1, 0, 1),
        time = Math.max(oParams.date || 0, oParams.time || 0) || Math.floor((ny.getTime() - Date.now()) / 1000),
        oCountdown = new window.webapp.Countdown($('.b-countdown').eq(0), time);
});
...
(function($) {
    ...
    function Countdown(el, time) {
        $.extend(this, {
            ...
            aNames: ['days', 'hours', 'minutes', 'seconds'],
            oItems: {}
        });
        this.initialize();
        this.tick();
    ...
    Countdown.prototype.tick = function() {
        ...
        var that = this,
            ...
            oTime = this.getTimeObj(this.time - 1),
            aDeferred = [];
